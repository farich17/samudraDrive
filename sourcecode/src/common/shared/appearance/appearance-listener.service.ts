...
import { extractRootDomain } from '../../core/utils/extractRootDomain';
...
export class AppearanceListenerService {
    ...
    private listenForMessages() {
        window.addEventListener('message', e => {
            if (this.eventIsTrusted(e) && this.isAppearanceEvent(e)) {
    ...
    private eventIsTrusted(e: MessageEvent): boolean {
        return extractRootDomain(e.origin) === window.location.hostname;
    }
